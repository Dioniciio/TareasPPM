...
int main()
{
    ...
    system("pause");  // Reemplazado read -p
    return 0;
}
...
void loop(char campo[V][H], int tam)
{
    ...
    do
    {
        system("cls");  // Reemplazado clear
        draw(campo);
        input(campo, &tam, &muerto);
        update(campo, tam);
        _sleep(100);  // Controla la velocidad de la serpiente (en milisegundos)
    } while (muerto == 0);
}
...
void input(char campo[V][H], int *tam, int *muerto)
{
    ...
    if (*muerto == 0)
    {
        if (kbhit())
        {
            key = getch();  // Cambié getchar() por getch() para no requerir Enter
